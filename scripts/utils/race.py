...
class Race:
    def __init__(
        ...
        self.race_info.ran = (
            self.race_info.ran if self.race_info.ran else str(len(self.runner_info.num))
        )
        ...
        self.runner_info.ofr = strip_row(
            self.doc.xpath('//td[@data-ending="OR"]/text()')
        )
        self.runner_info.rpr = strip_row(
            self.doc.xpath('//td[@data-ending="RPR"]/text()')
        )
        self.runner_info.ts = strip_row(
            self.doc.xpath('//td[@data-ending="TS"]/text()')
        )
        self.runner_info.silk_url = self.doc.xpath(
            '//img[@class="rp-horseTable__silk"]/@src'
        )
    ...
    def create_csv_data(self, fields: list[str]) -> list[str]:
        ...
        for field in fields:
            ...
            if hasattr(self.race_info, actual_field):
                ...
                runner_values.append(
                    [str(v) for v in getattr(self.runner_info, actual_field)]
                )
        ...
        for row in zip(*runner_values, strict=False):
            rows.append(
                race_prefix + "," + ",".join(row) if race_prefix else ",".join(row)
            )
        return rows
    ...
    def get_ids_trainer(self) -> list[str]:
        trainer_ids = self.doc.xpath(
            "//a[@data-test-selector='link-trainerName']/@href"
        )
    ...
    def get_names_horse(self) -> list[str]:
        ...
        return [
            f"{clean_string(horse)} {nat}" for horse, nat in zip(horses, nationalities)
        ]
    ...
    def get_positions(self) -> list[str]:
        positions = self.doc.xpath(
            "//span[@data-test-selector='text-horsePosition']/text()"
        )
        del positions[1::2]
    ...
    def get_race_pattern(self) -> str:
        ...
        if (
            "Forte Mile" in self.race_info.race_name
            and "(Group" in self.race_info.race_name
        ):
            return "Group 2"

        if any(
            x in self.race_info.race_name.lower() for x in {"listed race", "(listed"}
        ):
            return "Listed"
