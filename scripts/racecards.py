...
def validate_days_range(value: str, max_days: int) -> int:
    try:
        ...
        raise argparse.ArgumentTypeError(
            f"Invalid value: '{value}'. Expected an integer."
        )
...
def get_race_urls(
    ...
    for date in dates:
        ...
        for meeting in doc.xpath("//section[@data-accordion-row]"):
            course = meeting.xpath(
                ".//span[contains(@class, 'RC-accordion__courseName')]"
            )[0]
            if valid_meeting(course.text_content().strip().lower()):
                for race in meeting.xpath(
    ...
    for runner_json in runners_json:
        ...
        if data_opts.get("fetch_profiles", True):
            try:
                ...
                print(
                    f"Failed to find profile: {runner_json['horseUid']} - {runner_json['horseName']}"
                )
        ...
        if should_include_group("performance"):
            ...
            runner.rpr = (
                runner_json["rpPostmark"] if runner_json["rpPostmark"] else None
            )
            runner.ts = runner_json["rpTopspeed"] if runner_json["rpTopspeed"] else None
            runner.ofr = (
                runner_json["officialRatingToday"]
                if runner_json["officialRatingToday"]
                else None
            )
        ...
        if should_include_group("silk"):
            ...
            runner.silk_url = (
                f"https://www.rp-assets.com/svg/{runner_json['silkImagePath']}.svg"
            )
        ...
        if should_include_group("stats") and stats:
            ...
            jockey_stats = (
                stats.jockeys[str(runner.jockey_id)]
                if str(runner.jockey_id) in stats.jockeys
                else {}
            )
            trainer_stats = (
...
def scrape_racecards(
    ...
    for race_id, href in tqdm(
        ...
        race.off_time = datetime.datetime.fromisoformat(
            race_meta["raceDatetime"]
        ).strftime("%H:%M")
        ...
        race.handicap = (
            race.rating_band is not None or "handicap" in race.race_name.lower()
        )
